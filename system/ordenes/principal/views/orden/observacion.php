...
<div class="container">
    <div class="bs-docs-example">
        <div class="descriptionForm">OBSERVACIONES</div>
        <?php if (Session::get('role') == 1 or Session::get('role') == 2) { ?>
        <div>
            <a href="<?php print PATH_NAV.'orden/observacioncrear/'.$this->idOrden; ?>" class="btn"><i class="icon-plus-sign"></i> Agregar</a>
        </div>
        <hr>
        <?php } ?>
        <table id="tabla-observaciones" class="table table-bordered table-hover">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Descripcion</th>
...
$(function(){
    buscar_observaciones(<?php print $this->idOrden; ?>);
});
</script>