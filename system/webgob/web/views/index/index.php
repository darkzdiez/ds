<section>
    <div id="containerNewsHome">
        <?php
        $categorySEARCH = array(14, 15, 16);
        foreach ($categorySEARCH as $category) {
            $value=$this->news[$category];
            for ($i=0; $i<=1;$i++) {
                $file=$value[$i]['location'] . $value[$i]['idarticle'] . '/' . $value[$i]['filename'];
                if ($i == 0) {
                    ...
                    $fileThumb='system/webgob/temp/thumbnail/' . $value[$i]['idarticle'] . '-170x170-'.$filename;
                    if(!file_exists($fileThumb)){
                        $obj = new Thumbnail();
                        $thumbnail = $obj->generateThumbnail('system/webgob/'.$value[$i]['location'] . $value[$i]['idarticle'] . '/' . $filename, $fileThumb, 170, 160);
                    }else{
                        $thumbnail=$fileThumb;
                    }
                    $dia=Date::format_date($value[$i]['release_date'], $format = DATETIME_ARRAY);
                    print '<article>'
                    . '<div class="noticias_inicio">'
                    . '<div class="der"></div>'
                    . '<div class="med">'
                    . '<header>'
                    . '<div class="fecha cufon_gudea">' . $dia['day'] . ' ' . $dia['month_compact'] . '<br>0 <a href="'.PATH_NAV.'noticias/more/'. $value[$i]['idarticle'] . '-' . Text::CleanURL($value[$i]['title']) . '">Comentario</a></div>'
                    . '<h3 class="titulo"><a href="'.PATH_NAV.'noticias/more/'. $value[$i]['idarticle'] . '-' . Text::CleanURL($value[$i]['title']) . '">' . $value[$i]['title'] . '</a></h3>'
                    . '</header>'
                    . '<div class="linea"></div>'
                    . '<div class="foto">';
                    if($thumbnail) {
                    ...
                    print '</div>'
                    . '<div class="categoria"><a href="#">' . $value[$i]['categoryTipe'] . '</a>, <a href="#">' . $value[$i]['categoryLocation'] . '</a> por <a href="#">Prensa</a></div>'
                    . '<p class="descripcion">' . Text::cut(strip_tags($value[$i]['summary']), 250, END) . '</p>'
                    . '<div class="leermas"><a href="'.PATH_NAV.'noticias/more/'. $value[$i]['idarticle'] . '-' . Text::CleanURL($value[$i]['title']) . '">Leer Mas...</a></div>'
                    . '</div>'
                    . '<div class="izq"></div>'
                    . '</div>'
                    . '</article>';
