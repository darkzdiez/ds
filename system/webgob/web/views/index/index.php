    ...
    <div id="containerNewsHome">
        ...
        foreach ($categorySEARCH as $category) {
            ...
            for ($i=1; $i<=3;$i++) {
                ...
                . '</tr>'
                . '<tr>'
                . '<td class="title"><a href="'.PATH_NAV.'noticias/more/'. $value[$i]['idarticle'] . '-' . Text::CleanURL($value[$i]['title']) . '" title="'.strip_tags(htmlspecialchars_decode($value[$i]['title'])).'">' . Text::cut(strip_tags(htmlspecialchars_decode($value[$i]['title'])), 120, END) . '</a></td>'
                . '<td class="title"><a href="'.PATH_NAV.'noticias/more/'. $value[$i+1]['idarticle'] . '-' . Text::CleanURL($value[$i+1]['title']) . '" title="'.strip_tags(htmlspecialchars_decode($value[$i+1]['title'])).'">' . Text::cut(strip_tags(htmlspecialchars_decode($value[$i+1]['title'])), 120, END) . '</a></td>'
                . '</tr>'
                . '<tr>'
